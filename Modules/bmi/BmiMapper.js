...
const BmiMapper = () => {
  const heightInCms = [
    142.2, 144.7, 147.3, 149.8, 152.4, 154.9, 157.4, 160.0, 162.5, 165.1, 167.6,
    170.1, 172.7, 175.2, 177.8, 180.3, 182.8, 185.4, 187.9, 190.5, 193.0, 195.5,
    198.1, 200.6, 203.2, 205.7, 208.2, 210.8,
  ];
  ...
  // in kgs
  const weightInKgs = [
    41, 45, 50, 54, 59, 64, 68, 73, 77, 82, 86, 91, 95, 100, 104, 109, 113, 118,
    122, 127, 132,
  ];
  ...
  const calculateBmi = (weight, height) =>
    Math.round((weight * 10000) / (height * height));
  ...
  const cmsToInches = (height) => {
    ...
    return `${heightInFeet}' ${
      Math.round((factorInc + Number.EPSILON) * 10) / 10
    }''`;
  };
  ...
  (() => {
    ...
    bmiTable = heightInCms.map((h) =>
      weightInKgs.map((w) => calculateBmi(w, h))
    );
